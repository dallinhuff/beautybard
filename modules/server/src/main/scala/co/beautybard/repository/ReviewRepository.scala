...
class ReviewRepositoryLive(override val sessionPool: Resource[IO, Session[IO]])
    extends ReviewRepository[IO]
    with SkunkRepository[IO, Review]:
...
object ReviewRepositoryLive:
  def make(sessionPool: Resource[IO, Session[IO]]): IO[ReviewRepository[IO]] =
    IO.delay(ReviewRepositoryLive(sessionPool))
